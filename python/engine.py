...
class Engine:
    ...
    def analyze_turn(self, board, player):
        ...
        4. part4_result：{'type': 'direct'|'searched', 'move': (r,c)}
             direct（己方可杀/对方必堵）→ 直接取候选第 1 点，不进搜索；
             其余（defense/fallback）  → deep_search 深度推演后取分支分最高点。
        """
        opp = 3 - player
        ...
        # 4. 深度推演结果
        if cand['type'] == 'direct':
            result['part4_result'] = {'type': 'direct', 'move': cand['points'][0] if cand['points'] else None}
        else:
            pts = cand['points']
            mv = self.deep_search.deep_search(board, pts, player)
            result['part4_result'] = {'type': 'searched', 'move': mv}
        return result
