...
这是核心。每条候选路径使用**温度预算 `T0=320`**（GUI 可选 160 / 240 / 320），对每个候选点递归推演，模拟落子按候选权重扣温：
...
> **可选神经网络**：安装 `numpy torch` 后可启用神经网络叶子评估/决策网络增强（实验性）。未安装或模型缺失时自动回退纯算法，不会崩。叶子网络默认尝试加载包内 `value.pt`；决策网络需 `DSH_GOMOKU_USE_NET=1`，两者独立。训练样本与决策网络权重不入库。
...
# 本地保留 lib/ 插件代码时，也验证 Node 适配器及结算回滚
node --test gomoku/tests/test_plugin.cjs
```

可选神经网络测试在缺少对应依赖时标记为跳过，核心规则测试照常执行。
