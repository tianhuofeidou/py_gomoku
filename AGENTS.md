...
- 强制状态统一使用当前决策方视角，用带符号整数编码：
  ...
  - `+2` 存在必胜、`-2` 存在必败：软证据；
  - `0` 无：没有发现证据。
- 同一决策节点先走硬规则：所有有效子分支都是硬必胜 → 节点硬必胜；所有有效子分支都是硬必败 → 节点硬必败。
- 其余情况下只让硬状态 ±1 向上传播：任一有效子分支为硬必败 → 节点为存在必败（硬败势优先于硬胜势）；否则任一有效子分支为硬必胜 → 节点为存在必胜；否则为无。
- `+2` / `-2` 是当前决策层的局部软证据，只由硬状态 ±1 产生，不再向父层传播；父层聚合时按无（0）处理。
- 例如 `[-1, -2, -1]` 必须聚合为 `-2`（因为有硬必败 -1），不能升级成 `-1`；`[-2, -2]` 没有硬状态，聚合为 `0`，不再传播为 `-2`；`[+1, -2]` 只看硬状态，聚合为 `+2`。
...
- 修改项目代码后，若改动影响算法行为，必须在同一次修改中同步更新根目录 `算法设计文档.md` 的对应章节，保证文档与实现一致；不得只改代码、不改算法文档。
...
- 升级版本的提交信息必须以版本号打头，格式为 `vX.Y.Z <描述>`，例如 `v1.1.1 修复根节点胜败判定与候选排序`。不得只在 Git 标签上体现版本号、提交信息却改用 `feat:`/`fix:`/`perf:` 类型前缀；`v1.4.0` 至 `v1.5.0` 期间出现过这种漂移，不作为范例。
- 不升级版本的提交继续使用类型前缀，例如 `docs:`、`ci:`。
